    ...
    delJSON() {
        myData.cities.forEach(element => {
            if(element["name"] == this.name){
                ...
                myData.cities = myData.cities.filter(value => Object.keys(value).length !== 0);
            }
            else{
                element.softwareHouses.forEach(house =>{
                    if(house["name"] == this.name){
                        ...
                        myData.cities[1].softwareHouses = myData.cities[1].softwareHouses.filter(value => Object.keys(value).length !== 0);
                    } 
                    else{
                        house.departments.forEach(department =>{
                            if(department["name"] == this.name){
                                ...
                                myData.cities[1].softwareHouses[1].departments = myData.cities[1].softwareHouses[1].departments.filter(value => Object.keys(value).length !== 0);
                            }
                            else(
                                department.employees.forEach(employee =>{
                                    if(employee["name"] == this.name){
                                        ...
                                        myData.cities[1].softwareHouses[1].departments[1].employees = myData.cities[1].softwareHouses[1].departments[1].employees.filter(value => Object.keys(value).length !== 0);
                                    }
                                })
                            )
                        })
